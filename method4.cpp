...
int procedure4(City* cities, int n, int w, int t, int i) {
            ...
            if (dist <= w) {
                ...
                if (stays_i == stays_j) {
                    ...
                    if (cities[i].stays == stays_j) {
                        //if ((cities[j].M + cities[j].cost) > t) return 0;
                        cities[i].unique = 1;
